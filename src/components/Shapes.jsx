...
export function CupComposition({ className }) {
  return (
    <svg className={className} viewBox="0 0 420 380" aria-hidden="true" focusable="false">
      ...
      <circle cx="238" cy="168" r="128" fill={C.mustard} />
      {/* Quart de cercle lilas */}
      <path d="M0 110 A110 110 0 0 1 110 0 L110 110 Z" fill={C.lilac} />
      {/* Petit carré sauge incliné */}
      <rect x="338" y="18" width="48" height="48" rx="6" fill={C.sage} transform="rotate(14 362 42)" />
      {/* Anse */}
      <circle cx="344" cy="226" r="36" fill="none" stroke={C.ink} strokeWidth="20" />
      {/* Tasse : demi-disque */}
      <path d="M104 190 H344 A120 120 0 0 1 104 190 Z" fill={C.tomato} />
...
const HERO_VARIANTS = [
  // Arche + cercle
  <>
    <path d="M40 220 V120 A80 80 0 0 1 200 120 V220 Z" fill={C.tomato} />
    <circle cx="200" cy="70" r="46" fill={C.mustard} />
    <rect x="150" y="170" width="90" height="50" rx="25" fill={C.ink} />
  </>,
  // Demi-cercles empilés
  <>
    <path d="M30 140 A90 90 0 0 1 210 140 Z" fill={C.sage} />
    <path d="M60 220 A90 90 0 0 1 240 220 Z" fill={C.lilac} />
    <circle cx="66" cy="54" r="30" fill={C.tomato} />
  </>,
  // Cercle + quart + trait ondulé
  <>
    <circle cx="140" cy="120" r="84" fill={C.mustard} />
    <path d="M150 220 A100 100 0 0 1 250 120 L250 220 Z" fill={C.tomato} />
  ...
  <>
    ...
    <circle cx="70" cy="60" r="36" fill={C.sage} />
  </>,
]
...
export function Squiggle({ className, color = C.mustard }) {
  return (
...
export function Asterisk({ size = 22, color = C.tomato }) {
  return (
...
export function BandShapes({ className }) {
  return (
    <svg className={className} viewBox="0 0 1200 300" preserveAspectRatio="xMidYMid slice" aria-hidden="true" focusable="false">
      <circle cx="80" cy="260" r="110" fill={C.tomato} />
      <path d="M1040 0 A120 120 0 0 0 1160 120 L1160 0 Z" fill={C.mustard} />
      <rect x="1080" y="200" width="56" height="56" rx="8" fill={C.lilac} transform="rotate(18 1108 228)" />
      <path d="M180 50 q18 -20 36 0 t36 0 t36 0" fill="none" stroke={C.sage} strokeWidth="8" strokeLinecap="round" />
    </svg>
  )
}
